...
# After the [dotnet new nunit --language VB] command has run:
# 1. ~/.nuget/packages contains nunit/
#    so it is important the current user is sandbox.
# 2. The /tmp/dojo dir has a vbproj file called dojo.vbproj
#    It is important it has this name, since that is the name of the
#    .vbproj file used in the start-point
...
mkdir /tmp/dojo
cd /tmp/dojo
dotnet new nunit --language VB -o .
